        ...
        <header-top :address='addressObj.address'>
            ...
            <template #right>
                <a class="header_login">
                    <span class="header_login_text" @click='gopath("/Login")'>登录|注册</span>
                </a>
            </template>
        </header-top>
        <!--首页导航-->
...
<script>
    import {GETADDRESS} from '../store/mutations_type'
    ...
    export default {
        ...
        methods:{
           async initsj(){
               let resulte=await this.$http.wrap.getPosition()
               if(resulte.code===0){
                   this.$store.dispatch('actSetAddress',resulte.data)
               }
           },
           gopath(url){
